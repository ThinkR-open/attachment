...
- [`att_from_examples()`](https://thinkr-open.github.io/attachment/reference/att_from_examples.md)
  ...
  no longer chokes on inline R inside roxygen2 markdown tags such as
  `@param x \`r
  helper(“x”)\``when`helper()`is a package-local function. Previously,`att_from_examples()`delegated to`roxygen2::parse_file()`, which evaluated inline R against`baseenv()`(roxygen2's fallback when no package env is registered) — base R is reachable, but package-local helpers are not, so each affected tag emitted`could
  not find function “helper”\`. Example detection now uses a focused
  regex over the source instead of the full roxygen2 tokenizer
  ([\#135](https://github.com/ThinkR-open/attachment/issues/135)).
...
- [`att_from_rscript()`](https://thinkr-open.github.io/attachment/reference/att_from_rscript.md)
  now walks the R syntax tree instead of matching the source text with
  regexes. Dependency detection no longer produces false positives on
  `::` inside string literals or comments (e.g. xpath
  `following-sibling::td`, CSS `label::after`,
  `sprintf('%s::plot()', ...)`)
  ([\#120](https://github.com/ThinkR-open/attachment/issues/120),
  [\#132](https://github.com/ThinkR-open/attachment/issues/132)).
- The AST walker safely handles “empty” call arguments (`x[, 1]`,
  `x[1, ]`, empty `switch` alternatives, missing `else` branches).
  Earlier development drafts of the walker crashed with
  `argument "el" is missing, with no default` on any script containing
  these patterns, which would have silently degraded every real-world
  script to the legacy regex fallback.

### Detection — new patterns recognised

- [`att_from_rscript()`](https://thinkr-open.github.io/attachment/reference/att_from_rscript.md)
  also recognises `use("pkg", ...)` (R \>= 4.4)
  ([\#128](https://github.com/ThinkR-open/attachment/issues/128)),
  `getFromNamespace("fn", "pkg")`,
  [`loadNamespace()`](https://rdrr.io/r/base/ns-load.html), and
  named-argument forms such as
  [`library(package = "pkg")`](https://rdrr.io/r/base/library.html) or
  [`requireNamespace("pkg", lib.loc = "...")`](https://rdrr.io/r/base/ns-load.html).
- Fully-qualified dependency-introducing calls such as
  [`base::library(pkg)`](https://rdrr.io/r/base/library.html),
  [`base::requireNamespace("pkg")`](https://rdrr.io/r/base/ns-load.html),
  and `methods::getFromNamespace(fn, "pkg")` are now honoured; the inner
  package is added to the dependency list.
- Introspection helpers
  ...
  [`attachNamespace()`](https://rdrr.io/r/base/ns-load.html)) are
  intentionally **not** treated as dependency introducers to avoid
  silently widening `Imports` on code that only uses them for feature
  detection.

### Detection — robustness

- [`att_from_rscript()`](https://thinkr-open.github.io/attachment/reference/att_from_rscript.md)
  gains an `encoding` argument (default `getOption("encoding")`) so
  scripts saved in Latin-1 / Windows-1252 are read with the system
  locale instead of being forced to UTF-8.
- [`att_from_rscript()`](https://thinkr-open.github.io/attachment/reference/att_from_rscript.md)
  now warns when a file fails to parse as valid R code and the legacy
  regex-based detector is used as a fallback, so broken scripts are no
  longer silently degraded.
- The legacy regex-based fallback detector now accepts underscores in
  package names (e.g. `my_pkg::fn`). Previously an underscore truncated
  the detected name to the portion after the underscore, so a single
  syntax error upstream could corrupt detection across the whole file.

### Vignettes & Rmd / Quarto

- [`att_from_rmds()`](https://thinkr-open.github.io/attachment/reference/att_from_rmds.md)
  infers the vignette engine from the files actually present: `quarto`
  is added when `.qmd` files are found, `rmarkdown` when `.Rmd` files
  are found, both when the directory mixes the two. Previously
  `rmarkdown` was forced in for `.qmd`-only projects
  ([\#131](https://github.com/ThinkR-open/attachment/issues/131)).
- [`att_from_rmd()`](https://thinkr-open.github.io/attachment/reference/att_from_rmd.md)
  /
  [`att_from_rmds()`](https://thinkr-open.github.io/attachment/reference/att_from_rmds.md):
  `inside_rmd` now defaults to `NULL` and is auto-detected via
  `knitr::opts_knit$get("out.format")`, so users no longer have to think
  about it
  ([\#106](https://github.com/ThinkR-open/attachment/issues/106)).

### Maintenance

- Dropped the [fusen](https://thinkr-open.github.io/fusen/) development
  workflow; `R/amend_with_config.R`, `R/create_dependencies_file.R`,
  `R/dependencies_file_text.R` and their tests are now hand-maintained.
- Fixed
  [`create_renv_for_dev()`](https://thinkr-open.github.io/attachment/reference/create_renv_for_dev.md)
  by removing `'renv'` from `folder_to_include`.
