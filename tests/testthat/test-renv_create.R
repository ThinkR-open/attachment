...
my_renv_golem <- create_renv_for_dev(
  path = dummypackage,
  install_if_missing = FALSE,
  dev_pkg = "golem",
  output = file.path(tmpdir, "for_golem.lock"))
my_renv_ <- create_renv_for_dev(
  path = dummypackage,
  install_if_missing = FALSE,
  output = file.path(tmpdir, "blank.lock"))

expect_true(file.exists(file.path(tmpdir, "for_golem.lock")))
expect_true(file.exists(file.path(tmpdir, "blank.lock")))
